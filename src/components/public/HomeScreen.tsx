      ...
      <div className="kit-home-panel">
          ...
          <div className="kit-home-feature-media">
            {featuredKits.map((product, index) => (
              <Image
                ...
                width={520}
                height={620}
                sizes="(max-width: 800px) 42vw, 24vw"
                data-kit={index + 1}
              />
            ))}
          </div>
          <p>Front and back kit imagery can turn the store into a sharper supporter destination.</p>
        </div>
        <div className="kit-collection" role="list" aria-label={`${prospect.club.name} merchandise`}>
          {featuredKits.map((product, index) => <Link href="/store" className="kit-product" data-kit={index + 1} role="listitem" key={product.id}>
            <span className="kit-product-index">0{index + 1}</span>
            <div className="kit-product-media"><Image src={product.image} alt={product.name} fill sizes="(max-width: 800px) 82vw, 20vw"/></div>
      ...
      <section className="recruit-cta">
        ...
        <div className="recruit-cta-content">
          ...
          <div className="recruit-cta-actions">
            ...
            <Link href="/tryouts" className="recruit-cta-button secondary">Tryouts</Link>
          </div>
        </div>
      </section>
    )}
