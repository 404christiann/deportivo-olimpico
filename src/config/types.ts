...
export interface Branding {
  ...
  accentColor?: string;
  /** Optional small-detail color for hairline borders/dividers on dark surfaces only. Falls back to `accentColor` (then `secondaryColor`) — most prospects won't set this separately. */
  detailColor?: string;
  crest: string;
...
export interface TryoutDate { id: string; label: string; date: string; time: string; location: string; notes?: string; }
export interface TryoutsContent {
  eyebrow: string;
  headline: [string, string];
  intro: string;
  image: string;
...
export interface Product { id: string; name: string; price: number; image: string; /** Optional back-of-garment photo, shown via a toggle in the product detail modal. */ backImage?: string; category: "jersey" | "training" | "accessory" | "youth"; sizes?: string[]; /** Marks this product as the store's featured/hero product, overriding the default (first in the array). At most one product should set this. */ featured?: boolean; }
