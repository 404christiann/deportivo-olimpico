    ...
    <div className="store-page">
      <section className="store-campaign">
        <div className="store-campaign-copy">
          ...
          <div className="store-campaign-proof" aria-label="Store preview details">
            <span><small>Collection</small><strong>2026</strong></span>
            <span><small>Views</small><strong>Front / back</strong></span>
            <span><small>Checkout</small><strong>Club-owned</strong></span>
          </div>
        </div>
        ...
        {featured && <button className={`store-featured-product${prospect.branding.storeHeroImage ? " is-hero-photo" : ""}`} onClick={() => openProduct(featured)} aria-label={`View ${featured.name} details`}>
          <span className={`store-featured-image${prospect.branding.storeHeroImage ? " is-hero-photo" : ""}`}>
            {prospect.branding.storeHeroImage ? <>
              <Image
                src={prospect.branding.storeHeroImage}
                alt={prospect.branding.storeHeroImageAlt ?? featured.name}
                fill
                preload
                sizes="(max-width: 800px) 94vw, 48vw"
              />
              <span className="store-featured-fade" aria-hidden="true"/>
            </> : <>
              <span className="store-featured-kit primary"><Image src={featured.image} alt={featured.name} fill preload sizes="(max-width: 800px) 46vw, 24vw"/></span>
              {featured.backImage && <span className="store-featured-kit secondary"><Image src={featured.backImage} alt={`${featured.name} back`} fill sizes="(max-width: 800px) 40vw, 20vw"/></span>}
            </>}
          </span>
          <span className="store-featured-footer"><span><small>Featured kit</small><strong>{featured.name}</strong></span><b>${featured.price}</b></span>
        </button>}
      </section>
        ...
        <div className="store-product-grid">
          {products.map((product, index) => <button key={product.id} className="store-product-card" data-kit={index + 1} onClick={() => openProduct(product)}>
            ...
            <span className="store-product-image">
              <Image src={product.image} alt={product.name} fill sizes="(max-width: 700px) 58vw, 22vw"/>
              {product.backImage && <Image className="store-product-back" src={product.backImage} alt={`${product.name} back`} fill sizes="(max-width: 700px) 42vw, 16vw"/>}
            </span>
