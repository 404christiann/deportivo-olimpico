...
import type { Fixture, MatchResult, Player, Product, ProspectConfig, Season, Sponsor, StaffMember, StandingsRow, Team, TryoutDate, TryoutsContent } from "@/config/types";

export interface MockDataState {
  players: Player[]; staff: StaffMember[]; fixtures: Fixture[]; sponsors: Sponsor[]; products: Product[]; seasons: Season[]; teams: Team[]; standings: StandingsRow[]; tryouts: TryoutsContent;
}
...
type Action =
  ...
  | { type: "tryouts/update"; tryouts: TryoutsContent } | { type: "tryoutDate/add"; date: TryoutDate } | { type: "tryoutDate/update"; date: TryoutDate } | { type: "tryoutDate/remove"; id: string }
...
function reducer(state: MockDataState, action: Action): MockDataState {
  switch (action.type) {
    ...
    case "tryouts/update": return { ...state, tryouts: action.tryouts };
    case "tryoutDate/add": return { ...state, tryouts: { ...state.tryouts, dates: [...state.tryouts.dates, action.date] } };
    case "tryoutDate/update": return { ...state, tryouts: { ...state.tryouts, dates: state.tryouts.dates.map((item) => item.id === action.date.id ? action.date : item) } };
    case "tryoutDate/remove": return { ...state, tryouts: { ...state.tryouts, dates: state.tryouts.dates.filter((item) => item.id !== action.id) } };
...
function seed(config: ProspectConfig): MockDataState {
  const tryouts = config.tryouts ?? {
    eyebrow: "Open tryouts",
    headline: ["Earn your", "place."],
    intro: `Register interest for future ${config.club.shortName} tryout opportunities.`,
    image: config.branding.heroImage,
    imageAlt: config.branding.heroImageAlt,
    registrationHref: "/sponsors",
    registrationLabel: "Register interest",
    dates: [],
    details: ["Bring cleats, shin guards, water, and a valid ID.", "Arrive early for check-in.", "Club staff will follow up with selected players."],
  };
  return { players: config.roster, staff: config.staff, fixtures: config.fixtures, sponsors: config.sponsors, products: config.store.products, seasons: config.seasons, teams: config.teams, standings: config.standings?.rows ?? [], tryouts };
}
